import React from "react";
const RecentJob = () => {
    return (
        <div>
            <div className="bg-white w-full max-h-96">
                <h1 className="text-center pt-10 text-2xl  pb-10">Recent Jobs</h1>
                <div className="flex flex-wrap  items-center text-sm text-center w-4/5 h-28 bg-gray-100 m-0 mx-auto md:gap-16 lg:gap-32">
                    <div className="headings pt-4 ml-10">
                        <h1 className="text-xs font-semibold">Business Analyst</h1>
                        <p className="pt-4">Contract</p>
                    </div>
                    <div>
                        <h1 className="text-xs">
                            <span>
                                <i class="fas fa-map-marker-alt"></i>
                            </span>
                            Washington D.C, United
                        </h1>
                        <p>State</p>
                    </div>

                    <div className="text-xs">
                        <p>20001</p>
                    </div>

                    <div className="text-xs">
                        <p>Experience: 0 yrs</p>
                    </div>

                    <div>
                        <button className="bg-blue-600 text-white font-semibold w-24 h-8 rounded-lg hover:text-blue-500 hover:font-bold hover:bg-indigo-100">
                            Apply Now
                        </button>
                    </div>
                </div>
                <hr className="w-4/5 m-0 mx-auto mt-4 mb-4" />
            </div>

            <div className="bg-white w-full max-h-96">
                <div className="flex flex-wrap  items-center text-sm text-center w-4/5 h-28 bg-gray-100 m-0 mx-auto md:gap-16 lg:gap-32">
                    <div className="headings pt-4 ml-10">
                        <h1 className="text-xs font-semibold">Business Analyst</h1>
                        <p className="pt-4">Contract</p>
                    </div>
                    <div>
                        <h1 className="text-xs">
                            <span>
                                <i class="fas fa-map-marker-alt"></i>
                            </span>
                            Washington D.C, United
                        </h1>
                        <p>State</p>
                    </div>

                    <div className="text-xs">
                        <p>20001</p>
                    </div>

                    <div className="text-xs">
                        <p>Experience: 0 yrs</p>
                    </div>

                    <div>
                        <button className="bg-blue-600 text-white font-semibold w-24 h-8 rounded-lg hover:text-blue-500 hover:font-bold hover:bg-indigo-100">
                            Apply Now
                        </button>
                    </div>
                </div>
                <hr className="w-4/5 m-0 mx-auto mt-4 mb-4" />
            </div>

            <div className="bg-white w-full max-h-96">
                <div className="flex flex-wrap  items-center text-sm text-center w-4/5 h-28 bg-gray-100 m-0 mx-auto md:gap-16 lg:gap-32">
                    <div className="headings pt-4 ml-10">
                        <h1 className="text-xs font-semibold">Business Analyst</h1>
                        <p className="pt-4">Contract</p>
                    </div>
                    <div>
                        <h1 className="text-xs">
                            <span>
                                <i class="fas fa-map-marker-alt"></i>
                            </span>
                            Washington D.C, United
                        </h1>
                        <p>State</p>
                    </div>

                    <div className="text-xs">
                        <p>20001</p>
                    </div>

                    <div className="text-xs">
                        <p>Experience: 0 yrs</p>
                    </div>
                    <div>
                        <button className="bg-blue-600 text-white font-semibold w-24 h-8 rounded-lg hover:text-blue-500 hover:font-bold hover:bg-indigo-100">
                            Apply Now
                        </button>
                    </div>
                </div>
                <hr className="w-4/5 m-0 mx-auto mt-4 mb-4" />
            </div>
               
         <div className="text-center">
            <button className="w-24 rounded-xl bg-transparent border-2">Previous</button>
            <button className="w-16 rounded-xl text-blue-700 bg-transparent border-2 ml-2">Next</button>
        </div>

        </div>
    );
};

export default RecentJob;
